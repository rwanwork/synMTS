...
"Usage:  hmoment-topmedian.R --input INPUT --output OUTPUT --type IMGTYPE
...
x_long <- gather (x, key="measure", value="value", c("TopMedian"))
...
ggplot_obj <- ggplot_obj + theme (axis.title.y = element_blank (), strip.text.x = element_blank (), plot.title = element_text (hjust = 0.5))
ggplot_obj <- ggplot_obj + theme (legend.position="none")

##  Need a complicated way to combine text with a Greek letter
a = expression ("Median of top 10% of ")
b = expression (mu)
c = expression ("H")
e <- substitute (a * b * c, lapply (list (a = a, b = b, c = c), "[[", 1))
ggplot_obj <- ggplot_obj + labs (title = e)
