...
##  Positions in the DeepMito file
my $DEEPMITO_NAME = 0;
my $DEEPMITO_PROTEIN = 2;
my $DEEPMITO_PREDICTION = 3;
my $DEEPMITO_SCORE = 4;
my $DEEPMITO_GOTERM = 7;
...
open (my $mitofates_fp, "<", $deepmito_fn_arg) or die "EE\tCould not open $deepmito_fn_arg for input!\n";
while (<$mitofates_fp>) {
  my $line = $_;
  chomp ($line);
  $records_total++;
  ...
  my @tmp = split /\t/, $line;
  if ($tmp[$DEEPMITO_PROTEIN] ne $protein_arg) {
    next;
  }

  my $id = RemoveProtein ($tmp[$DEEPMITO_NAME]);
...
for (my $k = 0; $k < scalar (@output_order); $k++) {
  ...
  printf "%s", RemoveProtein ($tmp[$DEEPMITO_NAME]);
  printf "\t%s", CleanGOTerm ($tmp[$DEEPMITO_GOTERM]);
  printf "\t%s", $tmp[$DEEPMITO_SCORE];
  printf "\n";
}
