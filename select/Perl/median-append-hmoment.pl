...
$header = $header."\tMaxHMoment\tTopHMoment_median\tAllHMoment_median";
printf STDOUT "%s\n", $header;
...
while (<STDIN>) {
  ...
  printf STDOUT "\t%.3f", $curr_fp_tmp[$HMOMENT_TOP_MEDIAN_POS];
  printf STDOUT "\t%.3f", $curr_fp_tmp[$HMOMENT_ALL_MEDIAN_POS];
  printf STDOUT "\n";
