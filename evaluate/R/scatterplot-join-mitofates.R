...
"Usage:  scatterplot-join-mitofates.R --input INPUT --output OUTPUT --protein PROTEIN --type IMGTYPE
...
colnames (x) <- c ("Name", "Method", "Protein", "Original", "Substituted")
...
x$Method <- gsub ("4", "Method 4", x$Method)
x$Method <- gsub ("7", "Method 7", x$Method)
...
ggplot_obj <- ggplot (x, aes (x=Original, y=Substituted))
ggplot_obj <- ggplot_obj + geom_point()
ggplot_obj <- ggplot_obj + facet_wrap (~ Method, ncol=2)
