...
x <- read.table (in_fn, sep="\t", header=FALSE)
colnames (x) <- c ("Proteins", "Rank", "Count", "Proportion")

##  Remove the positive and negative controls
x <- x[x$Proteins != "Atp8p-Hac1p",]
x <- x[x$Proteins != "Atp9p-Hac1p",]
x <- x[x$Proteins != "Cox2p-Hac1p",]
x <- x[x$Proteins != "Atp8p-Mmf1p",]
x <- x[x$Proteins != "Atp9p-Mmf1p",]
x <- x[x$Proteins != "Cox2p-Mmf1p",]
x <- x[x$Proteins != "Hac1p-Mmf1p",]

##  Change Cox2p to the variant we used in the experiments
x$Proteins <- gsub ("-", "/", x$Proteins)
x$Proteins <- gsub ("Cox2p", "Cox2p-W56R", x$Proteins)
...
ggplot_obj <- ggplot_obj + geom_vline (xintercept = 250, linetype="dotted")
# ggplot_obj <- ggplot_obj + theme (legend.position = "inside", legend.position.inside = c(0.75, 0.25))
ggplot_obj <- ggplot_obj + theme (legend.position = "top", legend.title = element_blank (), text = element_text (size = 22))
ggplot_obj <- ggplot_obj + scale_y_continuous (limits = c(0, 1))
ggplot_obj <- ggplot_obj + scale_x_continuous (name = "Top N", breaks = seq(0, 1250, 250), limits = c(0, 1300))
