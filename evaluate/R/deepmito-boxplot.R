...
##  If other proteins are in x$Protein, then include the next line and comment out the one after
#x$Protein <- as.factor (x$Protein)
x$Protein <- factor (x$Protein, levels = c("Cox2p-W56R", "Atp9p", "Atp8p", "Hac1p", "Mmf1p"))

x_var <- "Protein"
y_var <- "Score"

ggplot_obj <- ggplot (x, aes (.data[[x_var]], .data[[y_var]]))
ggplot_obj <- ggplot_obj + geom_boxplot (position="dodge", aes (fill=Protein))
ggplot_obj <- ggplot_obj + xlab ("Protein") + ylab ("DeepMito Score")
ggplot_obj <- ggplot_obj + theme (legend.position = "none", text = element_text (size = 22))
#ggplot_obj <- ggplot_obj + coord_flip ()
